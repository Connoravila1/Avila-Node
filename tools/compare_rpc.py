...
def build_calls(height):
    return [
        ...
        # generatetodescriptor — getScriptFromDescriptor: arity+help,
        # collected -3 positions, getInt<int>/getInt<uint64_t> bounds,
        # parse -5, multipath -8, ranged -8, and nblocks<=0 → [].
        # (Result rows mine real blocks — excluded, heights diverge.)
        ("generatetodescriptor", []),
        ("generatetodescriptor", [1]),
        ("generatetodescriptor", [1, "x", 1, "x"]),
        ("generatetodescriptor", ["x", 1, "y"]),
        ("generatetodescriptor", [1.5, "raw(deadbeef)"]),
        ("generatetodescriptor", [2147483648, "raw(deadbeef)"]),
        ("generatetodescriptor", [1, "raw(deadbeef)", -1]),
        ("generatetodescriptor", [1, "raw(deadbeef)", 1.5]),
        ("generatetodescriptor", [1, "bogus"]),
        ("generatetodescriptor", [f"wpkh({TPUB}/<0;1>/*)#07eddr8t"]),
        ("generatetodescriptor", [f"wpkh({TPUB}/0/*)#f2s4pvjw"]),
        ("generatetodescriptor", [-1, "raw(deadbeef)"]),
        ("generatetodescriptor", [0, "raw(deadbeef)"]),
        ("getmininginfo", []),
