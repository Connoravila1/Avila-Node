...
fn fixtures() -> Vec<(Transaction, TxOut)> {
...
pub(super) fn run() -> Result<super::Outcome, String> {
    use avila_consensus::experimental_advice as advice;
    let cases = fixtures();
    let frame = advice::begin_block(&[0; 32], cases.len());
...
pub(super) fn pack(writer: &mut impl std::io::Write) -> std::io::Result<()> {
    avila_consensus::experimental_advice::pack_transactions(
        writer,
        &[0; 32],
