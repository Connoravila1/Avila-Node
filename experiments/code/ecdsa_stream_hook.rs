//! Isolated stream transport and one-pass arithmetic producer experiment.
...
pub fn configure(
    ...
    let trace = if matches!(mode, Mode::Capture | Mode::Produce) {
...
pub fn group_capacity() -> usize {
    SESSION
        .get()
        .filter(|s| {
            matches!(s.mode, Mode::Candidate | Mode::Stream | Mode::Produce)
...
pub fn group<T>(estimated_checks: usize, mut work: impl FnMut() -> (T, bool)) -> T {
    ...
    let mode = if (session.disabled.load(Ordering::Acquire) && session.mode != Mode::Produce)
...
pub fn verify(sig: &[u8], pubkey: &[u8], msg: &[u8; 32], ordinary: impl FnOnce() -> bool) -> bool {
    GROUP.with_borrow_mut(|slot| {
        ...
        group.ordinal += 1;
        if group.mode == Mode::Produce {
            let produced = canonical(sig, pubkey, msg).and_then(|record| group.produce(&record));
