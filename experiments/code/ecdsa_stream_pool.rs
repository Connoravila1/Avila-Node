        ...
        loop {
            ...
            let results = crate::experimental_advice::group(estimate, || {
                let results: Vec<_> = jobs.iter().map(|job| {
                    crate::experimental_advice::with_token(&job.advice, || {
    ...
    pub fn experimental_submit(&self, block: &crate::block::Block, jobs: Vec<(Transaction, Vec<TxOut>, crate::script::ScriptFlags)>)
        -> std::sync::Arc<BlockCheck> {
        let check = std::sync::Arc::new(BlockCheck::new(jobs.len()));
        let frame = crate::experimental_advice::begin_block(block.block_hash().as_bytes(), block.transactions.len() - 1);
        // Submit all at once to make the bounded group policy observable.
        let mut q = self.queue.lock().unwrap_or_else(|e| e.into_inner());
        for (index, (tx, outs, flags)) in jobs.into_iter().enumerate() {
            q.push_back(ScriptJob { tx, outs, flags, advice: crate::experimental_advice::token(&frame, index), check: std::sync::Arc::clone(&check) });
        }
