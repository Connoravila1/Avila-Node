...
| 35 | 09-24 | Erlay recon — pure-Rust minisketch | Is the sketch primitive tractable without C++ FFI? | **adopt (primitive) — works + measured** | `p2p/sketch.rs` + `p2p/recon.rs`: GF(2^32) syndromes + BM + trace-split decode, BIP-330 wire set + salted short-ids + round state machine. 512B sketch reconciles what 1.28MB inv sends; full round tested in-memory; spurious-decode contract documented. Decode quadratic — cap+rate-limit needed. Session/manager integration remains; intra-Avila first. | [erlay-sketch](2026-09-24-erlay-sketch.md) |
