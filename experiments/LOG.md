...
| 19 | 09-23 | ECDSA batch cost model (k256 primitives) | Can SP-batch beat libsecp's 92µs/sig? | **REJECTED** | batch-of-8 ≈43ms vs 736µs individual (~60× slower): k256 scalar mul alone (120µs) exceeds a whole libsecp verify; SP resultant ~41ms; substrate can't compete — any batch needs libsecp-internal arithmetic (C/unsafe, forbidden). ECDSA ~92µs/sig is near-optimal; the ~70% script floor is fixed | [probe](../crates/avila-consensus/examples/ecdsa_batch_probe.rs) |

| 18 | 09-23 | ECDSA batch feasibility (research spike) | Can standard (r,s)-only ECDSA batch-verify? | **feasible-bounded** | R-parity blocks naive MSM (literature: "no speed-ups" for standard ECDSA); SP algorithm (summation polys) gives ~2× at batch ≤9 — ~30% of sync wall; novel crypto, high risk; modified-ECDSA needs consensus change | [ecdsa-batch](2026-09-23-ecdsa-batch-feasibility.md) |
