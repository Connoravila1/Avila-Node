...
| P4 | Storage: chainstate/index/undo bytes, total footprint, reads/writes, write amplification | Aim for **half mutable-state/index overhead and write amplification**; compare retained block bytes separately | `coinsdb.redb` (redb): single-write-tx commits, sorted-key inserts; `CoinFormat::Compact` (Core's `Coin` codec — VARINT code + compressed amount + script type-ids) is the default for fresh databases — **halved coinsdb file** (2.0→1.0 GiB at 5M synthetic coins) and +12–25% bulk-commit throughput vs the legacy layout, point reads neutral, byte-identical UTXO state over the 501-block real segment (`experiments/2026-09-23-coinsdb-layout.md`). Write amplification vs Core's LevelDB compaction is **unmeasured** at mainnet scale |
...
- **Coins-engine experiment**: `experiments/2026-09-23-coinsdb-hash-engine.md`
  ...
  +97%, datadir −44% at 40M; at scale hash wins 2.0× ingest / 2.5×
  reads / 5.3× commits and the gap widens with depth (it loses only
  below ~1M coins, where the cached tree beats two preads). Opt-in,
  redb stays default.
