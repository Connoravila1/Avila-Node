...
| P4 | Storage: chainstate/index/undo bytes, total footprint, reads/writes, write amplification | Aim for **half mutable-state/index overhead and write amplification**; compare retained block bytes separately | `coinsdb.redb` (redb) landed: single-write-tx commits, ~190k coins/s inserts and ~405k point reads/s on the i3-N305 bench (`microbench` example); v3 inline state migrates on open; `state.dat` carries bookkeeping only. Write amplification vs Core's LevelDB compaction is **unmeasured** at mainnet scale |
