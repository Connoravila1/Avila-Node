...
mod tests {
    ...
    fn regtest() -> Chainstate {
        Chainstate::new(&Network::Regtest.params())
    }

    /// A chain of `n` blocks on `cs`'s genesis (headers indexed, bodies held
    /// by the caller, not yet submitted).
    fn chain_blocks(cs: &Chainstate, n: u32) -> Vec<Block> {
